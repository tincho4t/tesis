...
unsigned long long ticks = 0; // Contados de ticks en el partido
...
void saveScreen(void)
{
    ...
    int goal_team_a = 0;
    int goal_team_b = 0;
    if(game_status==S_GOAL_TEAM_A)
    {
        goal_team_a = 1;
    } else if(game_status==S_GOAL_TEAM_B)
    {
        goal_team_b = 1;
    }

	//////////////CONSIGO LA PANTALLA/////////////////////////
	Uint8 currentScreen [200][320][3]; // Alto, ancho, RGB
	SDL_PixelFormat *fmt;
	SDL_Color *color;
	...
	for(y=0;y<screen->h;y++){
		for(x=0; x<screen->w;x++){
			Uint8 pixel = getPixel(screen, x, y);
			color=&fmt->palette->colors[pixel];

			currentScreen[y][x][0] = color->r;
			currentScreen[y][x][1] = color->g;
			currentScreen[y][x][2] = color->b;
			//El Alpha no lo guardo.

			/* Unlock the surface */
			
			//if(30 <= x && x <= 35 && 30 <= y && y <= 40)
			//    D(bug("Pixel Color-> Red: %d, Green: %d, Blue: %d. (x,y): (%d,%d)\n", color->r, color->g, color->b, x,y));
    ...
    fprintf(f, "{\"tick\": %llu, \"seconds\": %lu, \"miliseconds\": %lu, \"goal_team_a\": %d, \"goal_team_b\": %d,\r\n", ticks, now.tv_sec, now.tv_usec, goal_team_a, goal_team_b);
    fprintf(f, "\"screen\": ["); // abro screen
    int h = screen->h;
    int w = screen->w;
    
    for(y=0;y<h;y++){
		fprintf(f, "["); // Abro la lista de Y
		for(x=0; x<w;x++){
		    fprintf(f, "[%d,%d,%d]",currentScreen[y][x][0],currentScreen[y][x][1],currentScreen[y][x][2]);
		    if(x+1 < w) fprintf(f, ","); // No agrego la coma al ultimo
		}
		fprintf(f, "]"); // Cierro lista de Y
		if(y+1 < h) fprintf(f, ","); // No agrego la coma al ultimo
		fprintf(f, "\r\n");
    }
    fprintf(f, "]"); // Cierro screen
    fprintf(f,"},\n");// Cierro el json
    fclose(f);
}

struct timeval stop, start = (struct timeval){0}; // Para contar tiempo entre cada tick
...
void HandleReferee(void)
{
    ...
    g->Tick--;
    if(start.tv_usec == 0){
        D(bug("Tomo por defecto el tiempo porque no esta inicializado!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n\n\n"));
